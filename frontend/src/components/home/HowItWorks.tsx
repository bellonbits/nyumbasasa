...
const STEPS = [
  ...
  },
  {
    ...
    color: "bg-accent",
    shadow: "shadow-[0_4px_24px_rgba(249,124,0,0.25)]",
  },
  {
    step: "03",
    icon: KeyRound,
    ...
    shadow: "shadow-[0_4px_24px_rgba(16,185,129,0.25)]",
  },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-ink-muted max-w-md mx-auto leading-relaxed">
            Nyumbasasa makes renting simple, transparent, and accessible for every Kenyan.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20 relative">
          {/* Connector line */}
          <div className="hidden md:block absolute top-10 left-[calc(33.33%+1rem)] right-[calc(33.33%+1rem)] h-0.5 border-t-2 border-dashed border-surface-border" />

          {STEPS.map(({ step, icon: Icon, title, description, color, shadow }) => (
            <div key={step} className="group text-center relative">
              <div className={`w-20 h-20 ${color} ${shadow} rounded-2xl flex items-center justify-center mx-auto mb-5 group-hover:scale-110 transition-transform duration-300`}>
                <Icon className="w-9 h-9 text-white" />
              </div>
              <span className="text-xs font-bold text-ink-faint tracking-[0.2em] uppercase">Step {step}</span>
              <h3 className="text-base font-bold text-ink mt-1.5 mb-2">{title}</h3>
              <p className="text-ink-muted text-sm leading-relaxed max-w-xs mx-auto">{description}</p>
            </div>
          ))}
        </div>

        {/* ── Dark stats banner ──────────────────────────────── */}
        <div className="relative overflow-hidden bg-[#0f1117] rounded-3xl">
          ...
          <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-0">
            ...
            {/* Right — stats */}
            <div className="grid grid-cols-1 divide-y divide-white/[0.08] border-t lg:border-t-0 lg:border-l border-white/[0.08]">
              {STATS.map(({ value, label, sub }) => (
                <div key={label} className="px-10 py-8 flex items-center gap-6">
                  <p className="text-display-sm font-extrabold text-brand-400 flex-shrink-0 leading-none">{value}</p>
                  <div>
